<script lang="ts">
    ...
    async function updatePost(title: string, message: string, published: boolean){
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}api/posts/${data.postId}/author`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("jwtAuth")}`
...
<h1 class="text-4xl pl-4 mt-4">Update Post</h1>
<form 
    action="/"
    class="bg-neutral-200 m-8 p-5 rounded-xl flex flex-col border border-neutral-500 relative">
    ...
    <button
        ...
        onclick={(event) => {
            ...
            if (title.length > 255){
                ...
                updatePost(title, message, published);
            }
